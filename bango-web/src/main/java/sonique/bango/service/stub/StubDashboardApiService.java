...
public class StubDashboardApiService implements DashboardApiService {
    ...
    private Function<Queue, QueueDashboardEntry> toQueueDashboardEntries() {
        return queue -> {
            ...
            List<DomainServiceProblem> serviceProblems = serviceProblemRepository.searchForServiceProblems(SearchParametersDTO.withSearchProperties("queueId", queue.id().asLong(), Integer.MAX_VALUE, 0)).getData();
